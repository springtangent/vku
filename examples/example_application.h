...
public:
	...
	virtual bool create_pipeline_layouts()
	{
	...
	virtual bool create_pipelines()
	{
		auto vert_module = read_example_shader("triangle.vert.spv");

		if (!vert_module.is_valid()) {
			return false;
		}

		auto frag_module = read_example_shader("triangle.frag.spv");
	...
	virtual bool populate_command_buffer(uint32_t i)
	{
		...
		if (!populate_command_buffer_render_pass(i))
		{
			return false;
		}
	...
	inline vku::ShaderModule read_example_shader(const std::string& filename)
	{
		...
		auto shader_module_result = vku::create_shader_module(vkb_device, code);
